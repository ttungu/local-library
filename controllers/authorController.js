...
exports.author_create_post = [
    ...
    body('family_name').trim().isLength({ min: 1 }).withMessage('Famliy name must be specified.')
        .isAlphanumeric().withMessage('Family name must be alpha-numeric.'),
...
exports.author_update_get = function (req, res, next) {
    Author.findById(req.params.id)
        .exec(function(err, author ){
            if(err) {return next(err)}
            res.render("author_form", {title: "Update "+author.name, author:author})
        })
};
